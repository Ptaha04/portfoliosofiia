...
let lang = 'pl';
const galleryState = { open: false, cat: 'all', filtered: [], lbIdx: 0 };
...
function setLang(newLang, data) {
  ...
  if (data) {
    renderShowcase(data);
    buildCatFilter(data);
    if (galleryState.open) filterGallery(galleryState.cat, data);
  }
}
...
function flatProjects(data) {
  return data.categories.flatMap(cat => cat.works.map(w => ({
    id: w.id,
    cat: cat.id,
    title: w.title || '',
    type: w.type || 'image'
...
function updateHeroStats(data) {
  ...
  $('#heroCats').textContent = data.categories.length;
  $('#countNum').textContent = projects.length;
}
...
function renderGalleryGrid(items, data) {
  ...
  items.forEach((p, idx) => {
    ...
    item.innerHTML = `
      <img class="gallery-item__img" src="${gdUrl(p.id)}" alt="${p.title || catLabel}" loading="lazy">
      ${isVideo ? `<div class="video-badge" aria-hidden="true"><svg viewBox="0 0 24 24" fill="currentColor" width="20" height="20"><polygon points="5,3 19,12 5,21"/></svg></div>` : ''}
...
function openFullGallery(cat, data) {
  ...
  $('#fullGalleryWrap').classList.add('is-visible');
...
function closeFullGallery() {
  ...
  $('#fullGalleryWrap').classList.remove('is-visible');
...
function filterGallery(cat, data) {
  ...
  const label = cat === 'all' ? `${t('cat.all')} — ${t('gallery.works')}` : getCatLabel(cat, data);
  $('#fullGalleryTitle').textContent = `${label} (${galleryState.filtered.length})`;
...
function openLightbox(idx) {
  ...
  $('#lbClose').innerHTML = t('lb.close');
  $('#lbCounter').textContent = `${idx + 1} / ${galleryState.filtered.length}`;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  $$('.lang__btn').forEach(btn => {
    btn.addEventListener('click', () => setLang(btn.dataset.lang, window._portfolioData));
  });
  ...
  renderShowcase(data);
  $('#btnShowAll')?.addEventListener('click', () => openFullGallery('all', data));
  $('#btnBackShowcase')?.addEventListener('click', closeFullGallery);
  buildCatFilter(data);
